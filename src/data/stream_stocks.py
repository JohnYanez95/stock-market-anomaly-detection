#!/usr/bin/env python3
"""
Stock Market Real-Time Streaming with Anomaly Detection

This module provides real-time stock market data streaming from Polygon.io
with built-in anomaly detection for equity markets during trading hours.

Key Features:
- Real-time stock data streaming (requires Starter tier, 15-min delayed)
- Volume anomaly detection optimized for equity volatility
- Market hours awareness (9:30 AM - 4:00 PM ET)
- Major stock symbols monitoring (AAPL, GOOGL, MSFT, TSLA, VOO)
...
Environment Requirements:
    POLYGON_TIER_STOCKS=starter  # Required for WebSocket access
"""
...
class PolygonWebSocketClient:
    ...
    def __init__(self, asset_type: AssetType = AssetType.STOCKS, api_key: str = None):
        ...
        # Log configuration
        delay_info = f"({self.config.get_delay_minutes(asset_type)}-minute delayed)" if not self.config.is_realtime_available(asset_type) else "(real-time)"
...
class RealTimeDataProcessor:
    ...
    def _process_aggregate(self, data: Dict):
        ...
        store_market_data('stocks', symbol, data)
...
def main():
    """Main function for stock market streaming"""
    ...
    try:
        logger.info("🚀 Starting stock market streaming...")
        logger.info("📈 Market hours: 9:30 AM - 4:00 PM ET (15-min delayed data)")
        ...
        logger.info("Stock streaming stopped by user")
    except Exception as e:
        logger.error(f"Error in stock streaming: {e}")
    finally:
        client.disconnect()
